...
impl<'a> DeviceHandle<'a> {
    ...
    pub unsafe fn new(c: PhantomData<&'a LibUsbContext>,
                      handle: *mut bindings::libusb_device_handle)
        -> DeviceHandle<'a> {
        DeviceHandle {
            _context: c,
    ...
    pub fn get_active_configuration(&self) -> Result<i32> {
        ...
        handle_libusb_error!(unsafe {
            bindings::libusb_get_configuration(self.handle, &mut config)
        });
        Ok(config as i32)
    }
    ...
    pub fn set_active_configuration(&mut self, config: i32) -> Result<()> {
        ...
        handle_libusb_error!(unsafe {
            bindings::libusb_set_configuration(self.handle, config as c_int)
        });
        Ok(())
    }
    ...
    pub fn claim_interface(&self, interface_number: i32) -> Result<()> {
        // Safe because 'self.handle' is a valid pointer to device handle.
        handle_libusb_error!(unsafe {
            bindings::libusb_claim_interface(self.handle, interface_number)
        });
        Ok(())
    }
    ...
    pub fn release_interface(&self, interface_number: i32) -> Result<()> {
        // Safe because 'self.handle' is a valid pointer to device handle.
        handle_libusb_error!(unsafe {
            bindings::libusb_release_interface(self.handle, interface_number)
        });
        Ok(())
    }
    ...
    pub fn reset_device(&self) -> Result<()> {
        // Safe because 'self.handle' is a valid pointer to device handle.
        handle_libusb_error!(unsafe {
            bindings::libusb_reset_device(self.handle)
        });
        Ok(())
    }
    ...
    pub fn kernel_driver_active(&self, interface_number: i32) -> Result<bool> {
        // Safe because 'self.handle' is a valid pointer to device handle.
        let v = handle_libusb_error!(unsafe {
