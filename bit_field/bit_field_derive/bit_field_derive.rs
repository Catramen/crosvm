...
// TODO(jkwang) Support custom mask?
// TODO(jkwang) Support custom field type?
...
fn get_struct_name(schema_name: &str) -> Result<&str> {
    let struct_name = schema_name.trim_right_matches("Schema");
    if struct_name.len() + "Schema".len() != schema_name.len() {
        return Err(format!("Check schema name, it should end with 'Schema'."));
    }
    if struct_name.len() == 0 {
        return Err(format!("Schema name is too short"));
    }
    Ok(struct_name)
}
...
fn get_fields_impl(fields: &[(String, Tokens)]) -> Vec<Tokens> {
    ...
    for &(ref name, ref ty) in fields {
        ...
        impls.push(quote!(
                pub fn #getter_ident(&mut self) -> <#ty as BitFieldSpecifier>::DefaultFieldType {
                    let offset = #(#ct0::FIELD_WIDTH as usize)+*;
                    return self.get(offset, #ty::FIELD_WIDTH) as <#ty as BitFieldSpecifier>::DefaultFieldType;
                }
...
mod tests {
    ...
    fn struct_name_parsing_valid() {
        match get_struct_name("MyBitFieldSchema") {
            Ok(name) => assert_eq!(name, "MyBitField"),
            Err(_e) => assert!(false),
        }
    }
    ...
    fn struct_name_parsing_invalid() {
        match get_struct_name("MyBitFieldS") {
            Ok(_name) => assert!(false),
            Err(_e) => (),
        }
    }
    ...
    fn end_to_end() {
        ...
        impl MyBitField {
            pub fn get_a(&mut self) -> <BitField1 as BitFieldSpecifier>::DefaultFieldType {
            ...
            pub fn get_b(&mut self) -> <BitField2 as BitFieldSpecifier>::DefaultFieldType {
            ...
            pub fn get_c(&mut self) -> <BitField5 as BitFieldSpecifier>::DefaultFieldType {
                let offset = BitField0::FIELD_WIDTH as usize
        ...
        mod tests {
            ...
            fn test_a() {
                ...
                a.set_a(BitField1::FIELD_MAX as <BitField1 as BitFieldSpecifier>::DefaultFieldType);
            ...
            fn test_b() {
                ...
                a.set_b(BitField2::FIELD_MAX as <BitField2 as BitFieldSpecifier>::DefaultFieldType);
            ...
            fn test_c() {
                ...
                a.set_c(BitField5::FIELD_MAX as <BitField5 as BitFieldSpecifier>::DefaultFieldType);
