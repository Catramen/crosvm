...
use msg_socket::{MsgOnSocket, MsgSocket, MsgError, MsgResult, MsgReceiver, MsgSender};
...
fn child_proc(sock: UnixDatagram, device: &mut BusDevice) {
    ...
    while running {
        ...
        let res = match cmd {
            ...
            Command::WriteConfig{ reg_idx, offset, len, data } => {
                let len = len as usize;
...
impl ProxyDevice {
    ...
    pub fn config_register_write(&mut self, reg_idx: usize, offset: u64, data: &[u8]) {
        ...
        self.sync_send(Command::WriteConfig{ reg_idx, offset, len, data: buffer });
    }
...
impl BusDevice for ProxyDevice {
    ...
    fn write(&mut self, offset: u64, data: &[u8]) {
        ...
        let len = data.len() as u32;
        buffer[0..data.len()].clone_from_slice(data);
        self.sync_send(Command::Write{ len, offset, data: buffer});
    }
}
