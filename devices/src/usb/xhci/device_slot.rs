...
impl DeviceSlot {
    ...
    /// configured.
    pub fn ring_doorbell(&self, target: u8, stream_id: u16) -> bool {
        debug!("Doorbell for device slot {}. target: {}, stream id: {}");
        if target < 1 || target > 31 {
            error!("Invalid target written to doorbell register. target: {}", target);
            return false;
        }
    ...
    pub fn disable() -> TrbCompletionCode {
        if self.enabled {
