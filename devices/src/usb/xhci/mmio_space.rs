...
use super::mmio_register::{
    BarOffset, BarRange, Register, RegisterInterface, RegisterSpec, StaticRegister,
    StaticRegisterSpec,
};
...
impl MMIOSpace {
    ...
    pub fn add_register_array<T>(&mut self, regs: &[Register<T>])
    where
...
mod tests {
    ...
    fn mmio_reg_writeable_callback() {
        ...
        reg.set_write_cb(move |val: u32| *state_clone.lock().unwrap() = val);
